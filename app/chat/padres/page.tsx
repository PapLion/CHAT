...
export default function ParentsChat() {
  ...
  useEffect(() => {
    ...
    if (userRole !== "parent" && userRole !== "authority") {
      router.push("/chat")
      return
    }
  ...
  const fetchMessages = async () => {
    try {
      const res = await fetch("/api/chats/padres/messages", {
        headers: {
    ...
    <div className="flex flex-col h-screen">
      <header className="bg-primary text-primary-foreground p-4">
        <div className="container mx-auto flex justify-between items-center">
          <h1 className="text-xl font-bold">Chat de Padres</h1>
          <Navigation />
        </div>
      </header>
